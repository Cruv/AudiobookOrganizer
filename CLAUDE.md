...
- **Trusted-provider field overwrite** (`services/candidates.py:apply_candidate`): when applying a candidate from a trusted provider (Audible/iTunes), series / series_position / year / narrator are ALWAYS overwritten on the book, not just filled-in-if-missing. Low-trust providers (Google/OpenLibrary) keep the old fill-if-missing behavior — they often have wrong audiobook-edition data.
- **Parser tightening** (`services/parser.py`): the 3-part nested-folder strategy now also rejects generic folder names as the author slot (`parts[-3]`), not just the series slot. Catches paths like `/library/Brandon Sanderson/The Final Empire` where "library" was getting through as author.
- 11 new tests: 2 for folder-only query + fallback, 3 for per-provider threshold behavior (Audible auto-applies at low score, Audible beats higher-scored OL, OL still works alone), 3 for trusted vs untrusted overwrite, plus existing test_candidates updated for the new scoring path.

### Recent Changes (Session 2026-05-15 part 12, v1.20.2 — duplicates page scale + error state)
- **Bug**: `/api/stats/duplicates` was loading every book in the DB with `selectinload(Book.candidates)` and grouping in Python — at thousands of books the response body got large enough that nginx truncated mid-stream (`ERR_CONTENT_LENGTH_MISMATCH` in the browser). The DuplicatesPage then sat on the loading skeleton forever because `isLoading || !data` also matched the error state.
- **Backend fix** (`routers/stats.py:get_duplicates`): two-phase fetch — Phase 1 is a SQL `GROUP BY LOWER(title), LOWER(author), edition HAVING COUNT(*) > 1` that returns only the keys with actual duplicates. Phase 2 fetches only the books matching those keys. Cover URLs come from a separate IN-query against `LookupCandidate` ordered by `applied DESC, ranking_score DESC`. Per-row serialization wrapped in try/except + logger.warning so one bad row can't 500 the whole response.
- **Frontend fix** (`pages/DuplicatesPage.tsx`): loading / error / ready are now distinct states. Errors render an `AlertTriangle` + message + Retry button instead of the infinite skeleton. Query `retry: false` so transient backend failures surface immediately.
...
- **Audio integrity check** (`services/integrity.py`, `routers/books.py:integrity_check_book`): per-`BookFile` columns `integrity_status` ('unchecked'|'ok'|'short'|'unreadable') + `integrity_detail`. New `POST /api/books/{id}/integrity-check` runs the fast check (mutagen open + size/duration ratio) and persists per-file status. Catches the most common failure mode — truncated downloads with intact headers but missing bytes.
- **Onboarding hint**: ScanPage shows a 3-step welcome card on first run (when there are no scans yet and none in-flight).
- 10 new tests covering ApiToken plaintext format, hash stability, prefix storage, create/list/revoke endpoints, name-required validation, and integrity check on missing/empty/garbage files.
